...
class SettingsRepository(private val settings: ObservableSettings) {
    ...
    val customBricks: Flow<List<CustomBrick>> = settings.getStringFlow(KEY_CUSTOM_BRICKS, "[]")
        .map { jsonString ->
            try {
                if (jsonString.isBlank()) emptyList<CustomBrick>()
                else json.decodeFromString<List<CustomBrick>>(jsonString)
            } catch (e: Exception) {
                emptyList<CustomBrick>() // Si falla, devolvemos lista vacía para no romper la app
            }
        }
    val hiddenBrickIds: Flow<Set<String>> = settings.getStringFlow(KEY_HIDDEN_BRICK_IDS, "[]")
        .map { jsonStr ->
            try {
                if (jsonStr.isBlank()) emptySet<String>()
                else json.decodeFromString<Set<String>>(jsonStr)
            } catch (e: Exception) {
                emptySet<String>()
            }
        }
    val customIrons: Flow<List<CustomIron>> = getListFlow<CustomIron>(KEY_CUSTOM_IRONS)
    val hiddenIronIds: Flow<Set<String>> = getSetFlow(KEY_HIDDEN_IRONS)
    val customRecipes: Flow<List<CustomRecipe>> = getListFlow<CustomRecipe>(KEY_CUSTOM_RECIPES)
    val hiddenRecipeIds: Flow<Set<String>> = getSetFlow(KEY_HIDDEN_RECIPES)
    val defaultBrickId: Flow<String> = settings.getStringFlow(KEY_DEF_BRICK, "")
    val defaultConcreteGenId: Flow<String> = settings.getStringFlow(KEY_DEF_CONCRETE_GEN, "")
    val defaultConcreteStrId: Flow<String> = settings.getStringFlow(KEY_DEF_CONCRETE_STR, "")
    val defaultPlasterRoughId: Flow<String> = settings.getStringFlow(KEY_DEF_PLASTER_ROUGH, "")
...
//        settings[KEY_CONTRAST] = mode.name
        settings.putString(KEY_CONTRAST, mode.name)
    }
    ...
    fun saveCustomBrick(brick: CustomBrick) {
        ...
        val currentList: MutableList<CustomBrick> = try {
            if (currentJson.isBlank()) mutableListOf<CustomBrick>()
            else json.decodeFromString<List<CustomBrick>>(currentJson).toMutableList()
        } catch (e: Exception) {
            mutableListOf<CustomBrick>()
        }
    ...
    fun saveCustomIron(item: CustomIron) = saveItemToList<CustomIron>(KEY_CUSTOM_IRONS, item) { it.id }
    ...
    fun saveCustomRecipe(item: CustomRecipe) = saveItemToList<CustomRecipe>(KEY_CUSTOM_RECIPES, item) { it.id }
